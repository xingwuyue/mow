  ...
  return {
    ...
    execute: function () {
      ...
      _export("PlayerRotation", PlayerRotation = (_dec = ccclass('PlayerRotation'), _dec2 = property({
        ...
        update(deltaTime) {
          var nearestEnemy = this._combat.getNearestEnemy();
        ...
        _rotateTowardsMovement() {
          var moveDir = this._movement.getMoveDirection();

          if (!moveDir) return; // 计算移动方向的角度

          var angle = Math.atan2(moveDir.y, moveDir.x) * 180 / Math.PI - 90; // 设置节点旋转
        ...
        _rotateTowards(targetPos) {
          ...
          var direction = new Vec3(targetPos.x - currentPos.x, targetPos.y - currentPos.y, 0); // 计算目标角度

          var angle = Math.atan2(direction.y, direction.x) * 180 / Math.PI - 90; // 设置节点旋转
