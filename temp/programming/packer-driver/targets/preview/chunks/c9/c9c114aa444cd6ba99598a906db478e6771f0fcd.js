    ...
    execute: function () {
      ...
      _export("PlayerController", PlayerController = (_dec = ccclass('PlayerController'), _dec2 = property({
        ...
        onLoad() {
          try {
            ...
            if (!this._attackRangeGraphics) {
              var graphicsNode = new Node('AttackRangeGraphics');
        ...
        shoot() {
          ...
          var targetPos = this._nearestEnemy.worldPosition; // 计算子弹到目标的方向

          var direction = new Vec3(targetPos.x - startPos.x, targetPos.y - startPos.y, 0).normalize(); // 从玩家位置偏移一定距离生成子弹

          var offsetDistance = 50;
          var spawnPos = new Vec3(startPos.x + direction.x * offsetDistance, startPos.y + direction.y * offsetDistance, 0);
        ...
        updateTouchInfo(event) {
          var camera = this.node.scene.getComponentInChildren(Camera);
          if (!camera) return;
          ...
          var dx = worldPos.x - nodePos.x;
          var dy = worldPos.y - nodePos.y;
        ...
        handleRotation(deltaTime) {
          if (this._nearestEnemy && this.isEnemyInRange(this._nearestEnemy)) {
            var enemyPos = this._nearestEnemy.worldPosition;
          ...
          var angleDiff = this._targetAngle - this._currentAngle;
          if (angleDiff > 180) angleDiff -= 360;else if (angleDiff < -180) angleDiff += 360;
          var rotationAmount = this.rotationSpeed * deltaTime;
        ...
        handleMovement(deltaTime) {
          var nodePos = this.node.getWorldPosition();
          var distance = Math.sqrt(Math.pow(this._touchPos.x - nodePos.x, 2) + Math.pow(this._touchPos.y - nodePos.y, 2));

          if (distance > this.stopDistance) {
            var moveAmount = this.moveSpeed * deltaTime;
            var newPos = this.node.position.clone();
            var nextX = newPos.x + this._moveDirection.x * moveAmount;
            var nextY = newPos.y + this._moveDirection.y * moveAmount; // 分别处理 X 和 Y 方向的移动，允许单方向移动
        ...
        handleCombat(deltaTime) {
          var enemies = this.node.scene.children.filter(node => {
            var enemy = node.getComponent(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
              error: Error()
            }), Enemy) : Enemy);
          ...
          if (enemies) {
            var playerPos = this.node.worldPosition;

            for (var enemy of enemies) {
              var distance = Vec3.distance(playerPos, enemy.worldPosition);
        ...
        findNearestEnemy() {
          ...
          var allNodes = getAllNodes(this.node.scene);
          var enemies = allNodes.filter(node => {
            var enemy = node.getComponent(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
              error: Error()
            }), Enemy) : Enemy);
            return enemy && !enemy.isDead();
          ...
          var minDistance = Vec3.distance(this.node.worldPosition, nearest.worldPosition);
        ...
        isEnemyInRange(enemyNode) {
          ...
          var distance = Vec3.distance(this.node.position, enemyNode.position);
