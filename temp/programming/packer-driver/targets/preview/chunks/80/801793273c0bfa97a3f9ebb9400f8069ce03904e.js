  ...
  return {
    ...
    execute: function () {
      ...
      _export("PlayerMovement", PlayerMovement = (_dec = ccclass('PlayerMovement'), _dec2 = property({
        ...
        _onTouchMove(event) {
          var touchPos = event.getUILocation();
          var delta = new Vec2(touchPos.x - event.getStartLocation().x, touchPos.y - event.getStartLocation().y);
        ...
        update(deltaTime) {
          ...
          var moveStep = this.moveSpeed * deltaTime; // 检查是否被阻挡，如果被阻挡则计算可移动的分量

          var moveVec = new Vec2(this._moveDirection.x, this._moveDirection.y);

          for (var blockedDir of this._blockedDirections) {
            var dot = Vec2.dot(moveVec, blockedDir);

            if (dot > 0.7) {
              // 调整阈值，使碰撞更自然
              // 计算平行于碰撞面的移动分量
              var parallel = new Vec2(-blockedDir.y, blockedDir.x);
              var parallelDot = Vec2.dot(moveVec, parallel); // 更新移动方向为平行分量
          ...
          var newPos = new Vec3(Math.max(this.MAP_BOUNDS.minX, Math.min(this.MAP_BOUNDS.maxX, currentPos.x + this._moveDirection.x * moveStep)), Math.max(this.MAP_BOUNDS.minY, Math.min(this.MAP_BOUNDS.maxY, currentPos.y + this._moveDirection.y * moveStep)), currentPos.z);
        ...
        onBeginContact(selfCollider, otherCollider, contact) {
          if (otherCollider.node.getComponent('Enemy')) {
            var currentPos = this.node.getWorldPosition();
            var enemyPos = otherCollider.node.getWorldPosition(); // 计算碰撞方向

            var direction = new Vec2(enemyPos.x - currentPos.x, enemyPos.y - currentPos.y).normalize();
