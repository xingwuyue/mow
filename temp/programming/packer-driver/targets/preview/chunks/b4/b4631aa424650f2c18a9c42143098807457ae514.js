    ...
    execute: function () {
      ...
      _export("Bullet", Bullet = (_dec = ccclass('Bullet'), _dec2 = property({
        ...
        update(deltaTime) {
          ...
          var moveAmount = this.speed * deltaTime * 2; // 将速度翻倍

          var currentPos = this.node.position;
          var newPos = new Vec3(currentPos.x + this._direction.x * moveAmount, currentPos.y + this._direction.y * moveAmount, 0);
        ...
        checkCollision() {
          ...
          var bulletBBox = new Rect(bulletPos.x - 15, bulletPos.y - 15, 30, 30);
          var enemies = ((_this$node$parent = this.node.parent) == null ? void 0 : _this$node$parent.getComponentsInChildren(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
            error: Error()
          }), Enemy) : Enemy)) || [];
          ...
          for (var enemy of enemies) {
            ...
            var enemyPos = enemyNode.worldPosition;
            var enemyBBox = new Rect(enemyPos.x - 25, enemyPos.y - 25, 50, 50);

            if (bulletBBox.intersects(enemyBBox)) {
              // 创建击中特效
              if (this.hitEffectPrefab) {
                var effect = instantiate(this.hitEffectPrefab);
                effect.parent = this.node.parent;
                var effectPos = new Vec3(bulletPos.x, bulletPos.y, 0);
