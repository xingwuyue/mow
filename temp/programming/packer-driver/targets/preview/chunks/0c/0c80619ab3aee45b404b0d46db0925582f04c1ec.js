    ...
    execute: function () {
      ...
      _export("PlayerCombat", PlayerCombat = (_dec = ccclass('PlayerCombat'), _dec2 = property({
        ...
        _updateNearestEnemy() {
          ...
          var nearestEnemy = this._findNearestEnemy(); // 如果当前目标无效或找到更近的目标，则更新目标
        ...
        _isCloserEnemy(newEnemy) {
          ...
          var currentTargetDist = this._currentTarget ? Vec3.distance(currentPos, this._currentTarget.getWorldPosition()) : Infinity;
          var newEnemyDist = Vec3.distance(currentPos, newEnemy.getWorldPosition());
          return newEnemyDist < currentTargetDist;
        } // 寻找范围内最近的敌人


        _findNearestEnemy() {
          var enemies = (_crd && EnemyManager === void 0 ? (_reportPossibleCrUseOfEnemyManager({
            error: Error()
          }), EnemyManager) : EnemyManager).instance.getEnemies();
          if (!enemies || enemies.length === 0) return null;
          var currentPos = this.node.getWorldPosition();
          var nearestEnemy = null;
          var minDistance = this.attackRange;

          for (var enemy of enemies) {
            if (!enemy.isValid) continue;
            var enemyPos = enemy.getWorldPosition();
            var distance = Vec3.distance(currentPos, enemyPos);
        ...
        _shootBullet() {
          ...
          var direction = new Vec3(targetPos.x - currentPos.x, targetPos.y - currentPos.y, 0).normalize(); // 设置子弹旋转

          var angle = Math.atan2(direction.y, direction.x) * 180 / Math.PI - 90;
