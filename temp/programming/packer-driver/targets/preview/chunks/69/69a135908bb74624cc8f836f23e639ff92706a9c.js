    ...
    execute: function () {
      ...
      _export("HealthManager", HealthManager = (_dec = ccclass('HealthManager'), _dec2 = property({
        ...
        initializeHealthBars() {
          ...
          for (var i = 1; i <= 8; i++) {
            var soldierName = i < 10 ? "00" + i : "0" + i;
            var soldier = this.node.getChildByName(soldierName);

            if (soldier) {
              // 创建血条实例
              var healthBar = instantiate(this.healthPrefab);
              canvas.addChild(healthBar); // 初始化血条

              var healthComp = healthBar.getComponent(_crd && PlayerHealth === void 0 ? (_reportPossibleCrUseOfPlayerHealth({
                error: Error()
