    ...
    execute: function () {
      ...
      _export("Bullet", Bullet = (_dec = ccclass('Bullet'), _dec2 = property({
        ...
        update(deltaTime) {
          ...
          // 每帧更新子弹位置
          var moveAmount = this.speed * deltaTime;
          ...
          if (collider) {
            var point = new Vec2(currentPos.x, currentPos.y);
            var results = PhysicsSystem2D.instance.testPoint(point);

            var _loop = function _loop() {
              if (result.group === 4) {
                var enemy = result.getComponent(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
                  error: Error()
                }), Enemy) : Enemy);

                if (enemy) {
                  enemy.takeDamage(_this.damage); // 创建命中特效

                  if (_this.hitEffectPrefab) {
                    var _hitEffect$getCompone;

                    var hitEffect = instantiate(_this.hitEffectPrefab);
                    hitEffect.parent = _this.node.parent;
        ...
        onBeginContact(selfCollider, otherCollider, contact) {
          ...
          if (otherCollider.group === 4) {
            var enemy = otherCollider.getComponent(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
              error: Error()
            }), Enemy) : Enemy);
            ...
            if (enemy) {
              ...
              if (this.hitEffectPrefab) {
                var hitEffect = instantiate(this.hitEffectPrefab);
