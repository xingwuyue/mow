    ...
    execute: function () {
      ...
      _export("PlayerHealth", PlayerHealth = (_dec = ccclass('PlayerHealth'), _dec2 = property({
        ...
        updatePosition() {
          ...
          const worldPos = this._targetNode.worldPosition; // 获取主相机

          const mainCamera = this.node.scene.getComponentInChildren(Camera);
          if (!mainCamera) return; // 获取屏幕尺寸

          const visibleSize = view.getVisibleSize(); // 将世界坐标转换为屏幕坐标

          const screenPos = mainCamera.convertToUINode(worldPos, this.node.parent); // 设置血条位置
