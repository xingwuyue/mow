    ...
    execute: function () {
      ...
      _export("Enemy", Enemy = (_dec = ccclass('Enemy'), _dec2 = property({
        ...
        onLoad() {
          ...
          const playerNode = (_crd && EnemyManager === void 0 ? (_reportPossibleCrUseOfEnemyManager({
            error: Error()
        ...
        update(dt) {
          ...
          const newPos = new Vec3(currentPos.x + this._currentDirection.x * moveAmount, currentPos.y + this._currentDirection.y * moveAmount, currentPos.z);
          this.node.setWorldPosition(newPos);
          const endTime = performance.now(); //console.log(`Update time: ${endTime - startTime}ms`);
        }

        onBeginContact(selfCollider, otherCollider, contact) {
          // 检查碰撞对象是否有 PlayerCombat 组件
          const playerCombat = otherCollider.node.getComponent(_crd && PlayerCombat === void 0 ? (_reportPossibleCrUseOfPlayerCombat({
            error: Error()
          }), PlayerCombat) : PlayerCombat);
