...
export class TConfig<T extends IConfig> {
    ...
    public get(id: number | string): T {
        ...
        const element = this._map[id];
        if (element == null) {
            // cc.error(this._name + " TConfig.get fail, id:", id);
        }
    ...
    public find(param: Param): T {
        ...
        cc.error(this._name + " TConfig.find fail, key:", JSON.stringify(param));
        return null;
    }
    ...
    public filter(param: Param): T[] {
        ...
        if (results.length <= 0) {
            // cc.error(this._name + " TConfig.filter fail, key:", JSON.stringify(param));
        }
        return results;
    }
        ...
        array.sort((a: T, b: T): number => {
            for (const key in param) {
                ...
                if (aValue != null && bValue != null) {
                    ...
                    cc.warn(this._name + " TConfig.sort property null, key:", key, "id", a.id, b.id);
                }
            }
            return 0;
        });
