...
@ccclass
export default class MoreGoldView_IOS extends MVC.BaseView {
    ...
    public onClickTab(toggle: cc.Toggle, customData: any) {
        ...
        if (toggle.node.name == "toggle0" || toggle.node.name == "toggle1") {
            if (this._invitePanel) {
                Notifier.send(ListenID.Invite_Close);
            }
        }
    ...
    public onInvite() {
        ...
        UIManager.Open(Common_UIPath.InviteUI, MVC.eTransition.Default, MVC.eUILayer.Tips);
    }
}
