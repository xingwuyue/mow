...
@ccclass
export default class BtnRewardMultView_IOS extends cc.Component {
    @property({ type: ePos })
    type: number = 1;
    ...
    onReward() {
        ...
        if (this.type == 4) {
            // HD_MODULE.getNet().postGameEvent({ event_name: 'check_double', counter: 1 });
            Notifier.send(ListenID.Log_Event, { event_name: "check_double" });
        }
    }
    ...
    private _updateRewwardIcon() {
        ...
        if (this.type == 5) {
            if (this._ctrl._multRewardType == 3) {
                ...
                this.videoNode.getChildByName("text_mxfh").y = 10;
                ...
                this.videoNode.getChildByName("text_mxfh").y = 0;
            }
        }
    }
    ...
    private _updateBtnRelive() {
        ...
        let isMember = GameVoManager.getInstance.getIsMember() && this._data.getRewardType() == 4;
