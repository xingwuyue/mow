...
@ccclass
export default class StageView extends cc.Component {
    ...
    public updateView(itemPrefab: cc.Prefab) {
        if (!this.isInit) {
            let data = RewardTaskModel.getInstance.getStageTaskList();
            ...
            for (let i = 0; i < this.spawnCount; i++) {
                ...
                itemdata.updateInfo(i, TaskType.Stage);
            }
            this.isInit = true;
        }
    }
    ...
    public onRewardAll() {
        ...
        let lognum = 0;
        for (let i = 0; i < len; i++) {
            let taskdata = RewardTaskModel.getInstance.getStageTaskDataByIndex(i);
            if (taskdata.taskState == 1) {
                lognum++;
                RewardTaskModel.getInstance.setStageTaskStateByIndex(i, 2);
        ...
        if(lognum > 0){
            Notifier.send(ListenID.Log_Event, { event_name: "levelTask_Award", counter: lognum });
        }
    }
        ...
        for (let i = 0; i < items.length; ++i) {
            ...
            if (isDown) {
                if (viewPos.y < -buffer && items[i].node.y + offset < 0) {
                    ...
                    item.updateInfo(itemId, TaskType.Stage);
                }
            } else {
