...
import { RemindData } from "../../common/Common_Define";

//--------------------------------
let remindResName = "icon_xhd";
...
let EventDispatchRigisterFunc = (id: any, func: Function, caller: any) => {
    Notifier.changeListener(true, id, func, caller);
}
...
export default class RemindController {
    ...
    private _activeUpdateTimer() {
        EventDispatchRigisterFunc(ListenID.Rigister_Remind, this._rigisterRemind, this);
    ...
    private _rigisterRemind(data: RemindData) {
        ...
        let create = () => {
            ...
            if (data.type == 1) {
                getSpriteFunc("icon_common_point").then((spFrame) => {
    ...
    private _updateAllRemind() {
        for (let name in this._remindList) {
            this._updateRemind(name);
        }
    }
}